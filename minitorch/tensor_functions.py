...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        (t1,) = ctx.saved_values
        num = grad_output.f.mul_zip(grad_output, grad_output.f.exp_map(-t1))
        denom = grad_output.f.add_zip(grad_output._ensure_tensor(1.0), grad_output.f.exp_map(-t1))
        denom = grad_output.f.mul_zip(denom, denom)
        return grad_output.f.mul_zip(num, grad_output.f.inv_map(denom))
...
class Permute(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        ...
        for i, pos in enumerate(order):
            reversed_order[pos] = i
        return grad_output._new(grad_output._tensor.permute(*reversed_order)), 0.0

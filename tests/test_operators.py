...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    assert sigmoid(a) <= sigmoid(a + 1.0) if abs(a) > 15 else sigmoid(a) < sigmoid(a + 1.0)
...
def test_other(a: float) -> None:
    ...
    """
    if abs(a) > 1e-5:
        assert_close(inv(inv(a)), id(a))
    assert_close(neg(neg(a)), id(a))
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    """
    return assert_close(sum(ls1) + sum(ls2), sum(addLists(ls1, ls2)))
